...
struct SegTree { // segment tree beats
  int n;
  vector<int> maxi, maxi2;
  ...
  inline void combine(int id, int il, int ir) {
    if (maxi[il] == maxi[ir]) {
      maxi[id] = maxi[il];
      maxi2[id] = max(maxi2[il], maxi2[ir]);
    } else if (maxi[il] > maxi[ir]) {
      maxi[id] = maxi[il];
      maxi2[id] = max(maxi2[il], maxi[ir]);
    } else {
      maxi[id] = maxi[ir];
      maxi2[id] = max(maxi[il], maxi2[ir]);
    }
  }

  void update(int x, int y, int v, set<int>& changed, int id, int l, int r) {
    if (x >= r || l >= y) return;
    if (x <= l && r <= y && v > maxi2[id]) {
      if (maxi[id] > v) {
        changed.insert(maxi[id]);
        maxi[id] = v;
      }
      return;
    }
